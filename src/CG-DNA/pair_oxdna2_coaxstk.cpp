...
/* ----------------------------------------------------------------------
   compute function for oxDNA2 pair interactions
------------------------------------------------------------------------- */
...
void PairOxdna2Coaxstk::compute(int eflag, int vflag)
{
  ...
  double df2,df4f6t1,df4t1,df6t1,df4t4,df4t5,df4t6;
  double df4t4_1,df4t4_2,df4t5_1,df4t5_2,df4t6_1,df4t6_2;
  ...
  for (ia = 0; ia < anum; ia++) {
    ...
    for (ib = 0; ib < bnum; ib++) {
      ...
      f4t4 = F4(theta4, a_cxst4[atype][btype], theta_cxst4_0[atype][btype],
                ...
                dtheta_cxst4_c[atype][btype], df4t4_1) +
             F4(theta4, a_cxst4[atype][btype], MY_PI - theta_cxst4_0[atype][btype],
                dtheta_cxst4_ast[atype][btype], b_cxst4[atype][btype],
                dtheta_cxst4_c[atype][btype], df4t4_2);
      ...
      f4t5 = F4(theta5, a_cxst5[atype][btype], theta_cxst5_0[atype][btype],
                ...
                dtheta_cxst5_c[atype][btype], df4t5_1) +
             F4(theta5p, a_cxst5[atype][btype], theta_cxst5_0[atype][btype],
                dtheta_cxst5_ast[atype][btype], b_cxst5[atype][btype],
                dtheta_cxst5_c[atype][btype], df4t5_2);
      ...
      f4t6 = F4(theta6, a_cxst6[atype][btype], theta_cxst6_0[atype][btype],
                ...
                dtheta_cxst6_c[atype][btype], df4t6_1) +
             F4(theta6p, a_cxst6[atype][btype], theta_cxst6_0[atype][btype],
                dtheta_cxst6_ast[atype][btype], b_cxst6[atype][btype],
                dtheta_cxst6_c[atype][btype], df4t6_2);
      ...
      df4f6t1 = (df4t1 + df6t1)/sin(theta1);

      df4t4 = (df4t4_1 + df4t4_2)/sin(theta4);

      df4t5 = (df4t5_1 - df4t5_2)/sin(theta5);

      df4t6 = (df4t6_1 - df4t6_2)/sin(theta6);
      ...
      if (evflag) ev_tally_xyz(a,b,nlocal,newton_pair,evdwl,0.0,delf[0],delf[1],delf[2],
...
void PairOxdna2Coaxstk::coeff(int narg, char **arg)
{
  ...
  if (narg != 3 && narg != 21) error->all(FLERR,"Incorrect args for pair coefficients in oxdna2/coaxstk" + utils::errorurl(21));
    ...
    for (j = i; j <= atom->ntypes; j++) {
      ...
      if (setflag[i][j]) {
        if (me == 0) {
          ...
          utils::sfread(FLERR,&AA_cxst1[i][j],sizeof(double),1,fp,nullptr,error);
          utils::sfread(FLERR,&BB_cxst1[i][j],sizeof(double),1,fp,nullptr,error);
